...
//Stencil for sponge
real stencil_sponge_Z(const offset_t off, 
		      ...
		      const integer dimmx) 
{
	return((C0*(ptr[IDX(z  +off,x,y,dimmz,dimmx)] - ptr[IDX(z-1+off,x,y,dimmz,dimmx)]))*dzi);
};


real stencil_sponge_X( const offset_t off,
                       const real* restrict ptr,
       		       const real dxi,
       		       const integer z,
       		       const integer x,
       		       ...
       		       const integer dimmx)
{
	return ((C0*(ptr[IDX(z,x  +off,y,dimmz,dimmx)] - ptr[IDX(z,x-1+off,y,dimmz,dimmx)]))*dxi);};

real stencil_sponge_Y( const offset_t off,
		       const real* restrict ptr,
       		       const real dyi,
       		       const integer z,
       		       const integer x,
       		       ...
       		       const integer dimmx)
{
    return ((C0 * ( ptr[IDX(z,x,y  +off,dimmz,dimmx)] - ptr[IDX(z,x,y-1+off,dimmz,dimmx)]))*dyi);
};
